...
const (
	redisDedupKeyPrefix  = "chatify:push:kafka:dedup:msg:"
...
// 使用 Redis Set 集合存储消息ID，key格式为：chatify:push:kafka:dedup:msg:{msgId}
...
func (r *messageDedupRepo) CheckAndSetDedup(ctx context.Context, msgId string) (bool, error) {
	...
	// 格式化Redis key: chatify:push:kafka:dedup:msg:{msgId}
