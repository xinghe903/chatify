...
func (r *messageRepo) SaveMessages(ctx context.Context, messages []*po.Message) error {
	...
	if tx.Error != nil {
		r.log.Error("failed to begin transaction", tx.Error.Error())
		return tx.Error
	}
	...
	for i := 0; i < len(messages); i += batchSize {
		...
		if err := tx.Create(batch).Error; err != nil {
			tx.Rollback()
			r.log.Error("failed to save messages", err.Error())
			return err
		}
	}
	...
	r.log.Debug("save messages success", "count", len(messages))
	return nil
}