...
* 日志。集成日志功能，使用 EFK。Elasticsearch存储数据，Filebeat进行数据采集，Kibana进行数据可视化。
* 链路追踪。记录请求和响应，使用Jaeger。通过Opentelemetry协议将数据通过Jaeger存储在Elasticsearch，通过Jaeger进行可视化。
* 监控管理。提供监控功能。使用 Prometheus 和 Grafana。使用alertmanager进行告警。
...
![alt text](doc/pics/1662d22b6e35ab3b221f20d0921c6aef.png)

2. 链路追踪

![alt text](doc/pics/590c301822ce239fdeb19ea58ae2e3ad.png)

![alt text](doc/pics/72f7982421ec90f8a2076d5753601576.png)

3. 指标管理

![alt text](doc/pics/88ae7ea12b43fce3b98e3845f9d1631f.png)

4. 接收消息通知

![alt text](doc/pics/image001.png)


## 二、快速开始

项目采用docker-compose进行部署，依赖docker和docker-compose。

使用k6进行压测。[安装地址](https://github.com/grafana/k6/releases)下载最新的版本解压就能用。
...
> cd test; k6 run --vus 100 --duration  10m  sendSystemPush.js
...
* Access 服务在用户上线时数据由写入 Redis，下线时清除。
